output "bosh_subnet_id" {
  value = element(
    split(",", var.infra_subnet_ids),
    var.zone_index[var.bosh_az],
  )
}

output "bosh_subnet_cidr" {
  value = var.infra_cidrs[format("zone%s", var.zone_index[var.bosh_az])]
}

output "bosh_default_gw" {
  value = var.infra_gws[var.infra_cidrs[format("zone%s", var.zone_index[var.bosh_az])]]
}

output "microbosh_static_private_ip" {
  value = var.microbosh_ips[var.bosh_az]
}
...
output "bosh_az_label" {
  value = var.zone_labels[var.bosh_az]
}
