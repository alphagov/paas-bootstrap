...
# List all instances with their CA certificate

# Filter instances using the old CA certificate
echo "Instances still using the old CA certificate ($prev_ca_certificate):"
echo "$rds_instances" | jq -r --arg OLD_CA "$prev_ca_certificate" '.DBInstances[] | select(.CACertificateIdentifier == $prev_ca_certificate) | .DBInstanceIdentifier'
